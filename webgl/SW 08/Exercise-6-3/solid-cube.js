...
function SolidCube (gl, backColor, frontColor, rightColor, leftColor, topColor, bottomColor)
{
    function defineVertices(gl)
    {
        ...
        console.log (vertices.length, 'vertex coordinates')
    ...
    function defineSides (gl)
    {
        ...
        var vertexIndices =
        [
             0,  2,  1,    2,  0,  3,   // face 0 (back)
             4,  5,  6,    4,  6,  7,   // face 1 (front)
             8,  9, 10,   10, 11,  8,   // face 2 (right)
            12, 15, 14,   14, 13, 12,   // face 3 (left)
            16, 17, 18,   18, 19, 16,   // face 4 (top)
            20, 23, 22,   22, 21, 20    // face 5 (bottom)
        ];

        console.log (vertexIndices.length, 'indices')
        var buffer = gl.createBuffer ();
    ...
    function defineTextureCoord (gl)
    {
        ...
        console.log (textureCoords.length, 'texture coordinates')
        var buffer = gl.createBuffer();
        gl.bindBuffer (gl.ARRAY_BUFFER, buffer);
    ...
    function defineNormals (gl)
    {
        ...
        var allSidesNormal = backSideNormal.concat (frontSideNormal,
            rightSideNormal, leftSideNormal, topSideNormal, bottomSideNormal);

        console.log (allSidesNormal.length, 'normal coordinates')
        var buffer = gl.createBuffer();
        gl.bindBuffer (gl.ARRAY_BUFFER, buffer);
